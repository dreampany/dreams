...
public class TotalTraffic {
    ...
    public static void calcTraffic(Context context, VpnPref pref, long in, long out, long diffIn, long diffOut) {
        List<String> totalTraffic = getTotalTraffic(pref, diffIn, diffOut);
    ...
    public static List<String> getTotalTraffic(VpnPref pref) {
        return getTotalTraffic(pref, 0, 0);
    }

    public static List<String> getTotalTraffic(VpnPref pref, long in, long out) {
        List<String> totalTraffic = new ArrayList<String>();

        if (inTotal == 0)
            inTotal = pref.getDownload();

        if (outTotal == 0)
            outTotal = pref.getUpload();
    ...
    public static void saveTotal(VpnPref pref) {
        if (inTotal != 0)
            pref.setDownload(inTotal);

        if (outTotal != 0)
            pref.setUpload(outTotal);
    }
