...
public class PlayView extends View {
        ...
        for (int xx = 0; xx < game.numSquaresX; xx++) {
            for (int yy = 0; yy < game.numSquaresY; yy++) {
                ...
                if (currentTile != null) {
                    ...
                    List<AnimationCell> aArray = game.aGrid
                            .getAnimationCell(xx, yy);
                    boolean animated = false;
                    for (int i = aArray.size() - 1; i >= 0; i--) {
                        AnimationCell aCell = aArray.get(i);
                        // If this animation is not active, skip it
                        if (aCell.getAnimationType() == PlayGame.SPAWN_ANIMATION) {
                            animated = true;
                        }
                        if (!aCell.isActive()) {
                            continue;
                        ...
                        if (aCell.getAnimationType() == PlayGame.SPAWN_ANIMATION) { // Spawning
                            // animation
                        ...
                        } else if (aCell.getAnimationType() == PlayGame.MERGE_ANIMATION) { // Merging
                            // Animation
                            double percentDone = aCell.getPercentageDone();
                            ...
                            bitmapCell[index].setBounds(
                                    ...
                                    (int) (eY - cellScaleSize));
                            bitmapCell[index].draw(canvas);
                        } else if (aCell.getAnimationType() == PlayGame.MOVE_ANIMATION) { // Moving
                            // animation
                            double percentDone = aCell.getPercentageDone();
                            int tempIndex = index;
                            if (aArray.size() >= 2) {
    ...
    private void drawEndGameState(Canvas canvas) {
        ...
        for (AnimationCell animation : game.aGrid.globalAnimation) {
            if (animation.getAnimationType() == PlayGame.FADE_GLOBAL_ANIMATION) {
    ...
    private void initView() {
        ...
        // Loading resources
        game = new PlayGame(getContext(), this);
        try {
            // Getting text values
